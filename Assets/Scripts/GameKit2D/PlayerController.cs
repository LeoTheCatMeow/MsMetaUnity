...
namespace Gamekit2D {
	...
	public class PlayerController : MonoBehaviour {
		...
		public bool brawlerMovementEnabled;
		...
		public void Move(Vector3 movement) {
			//Added this condition for brawler movement
			// if( brawlerMovementEnabled && transform.position.y + movement.y < Camera.main.ScreenToWorldPoint(Vector3.zero).y )
			// movement.y = Camera.main.ScreenToWorldPoint(Vector3.zero).y - transform.position.y;
				...
				if( bottom ) {
					...
					for( int j = 0; j < m_HitBuffer.Length; j++ ) {
						if( m_HitBuffer[j].collider != null ) {
							//Condition for One Way Platforms
